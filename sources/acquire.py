"""
Acquire.com source module.
Uses Playwright to scrape app acquisition listings.
"""
...
def fetch() -> list[dict]:
    """Fetch app listings from Acquire.com using Playwright."""
    try:
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info("Acquire: navigating to search page")
            page.goto(ACQUIRE_URL, wait_until="networkidle", timeout=30000)
            ...
            try:
                ...
                logger.warning("Acquire: timeout waiting for listings. Trying anyway.")

            # Try multiple selectors for Acquire's listing cards
            listings = page.query_selector_all(
                "[class*='ListingCard'], [class*='listing-card'], "
                "[class*='startup-card'], [class*='search-result']"
            )

            if not listings:
                # Fallback: links to individual listings
                listings = page.query_selector_all(
                    "a[href*='/startup/'], a[href*='/listing/']"
                )
            ...
            for listing in listings[:MAX_RESULTS]:
                try:
                    ...
                    if link and not link.startswith("http"):
                        link = f"https://acquire.com{link}"

                    # Extract revenue
                    revenue_el = listing.query_selector(
                        "[class*='revenue'], [class*='mrr'], [class*='income']"
                    )
                    ...
                    items.append({
                        "name": name,
                        "source": "Acquire",
                        "source_url": link or ACQUIRE_URL,
                    ...
                    logger.error("Error parsing Acquire listing: %s", e)
                    continue

            browser.close()

    except Exception as e:
        logger.error("Acquire scraping failed: %s", e)

    logger.info("Acquire: fetched %d total items", len(items))
    return items
